...
export default function StoreSidebar({
  ...
  const { categories, brands, priceRanges, sortOptions } = STORE_PAGE_CONTENT;
  ...
  return (
    <aside className="space-y-8">
      ...
      {/* Brands */}
      <div>
        <h3 className="mb-4 text-lg font-bold text-white">Brands</h3>
        <div className="space-y-2">
          {brands.map((brand) => (
            <label
              key={brand.id}
            ...
            >
              <input
                ...
                className="h-4 w-4 rounded border-white/20 bg-white/5 text-amber-500 focus:ring-2 focus:ring-amber-500/20"
              />
